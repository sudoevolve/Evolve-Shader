...
## Example Screenshot
![Example Shader](2.png)
![Example Shader](1.png)
...
## 🧩 Windows + Visual Studio + vcpkg（推荐）

本项目依赖以下库（通过 vcpkg 安装最省事）：
- `glfw3`（窗口/输入）
- `glad`（OpenGL loader，对应 `#include <glad/glad.h>`）
- `stb`（提供 `stb_image.h`，对应 `#include "stb_image.h"`）
...
### 2) 安装依赖（按你的平台选择 triplet）

```powershell
.\vcpkg install glfw3 glad stb --triplet x64-windows
```

如果你在 VS 里用 Win32/x86 构建：

```powershell
.\vcpkg install glfw3 glad stb --triplet x86-windows
```

### 3) 在 Visual Studio 中构建

- 确保 VS 的平台选择与 triplet 一致（x64 对 x64-windows，Win32 对 x86-windows）
- 打开 [Evolve shader.sln](file:///c:/Users/Administrator/Documents/Evolve-Shader-main/Evolve%20shader.sln) 并直接 Build
...
3. **进入交互式配置模式**
   ...
   === Interactive iChannel Setup ===
    Press Enter for auto-chain (buffer1 -> buffer2 -> ...),
    or type a buffer index (1 to N) to configure manually: 
   ```
...
Enter up to 4 choices for iChannel0-3, separated by spaces.
  1: none
  2: self (feedback)
  3: buffer2
...
| `3~N+1` | 使用其他 pass 的输出缓冲区（列表会排除当前 pass 自己） |
| `N+2+` | 使用 `iChannel/` 中的图像文件 |

示例：若你有 2 个 shader，则存在 `buffer1` 和 `buffer2`；图像选项从 `(base=N+2)` 开始（有 2 个 shader 时 base=4）。
